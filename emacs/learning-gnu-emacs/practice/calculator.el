...
(defvar calc-mode-map nil "Local keymap for calculator mode buffers.") ;for C-h v

;; set up the calculator mode keymap with C-j (linefeed) as "eval" key
(if calc-mode-map
    nil
  (setq calc-mode-map (make-sparse-keymap))
  (define-key calc-mode-map "\C-j" 'calc-eval))

(defconst calc-number-regexp "-?\\([0-9]+\\.?\\|\\.\\)[0-9]*\\(e[0-9]+\\)?"
...
(defun calc-pop ()
  (let ((val (calc-top)))
    (if val (setq calc-stack (cdr calc-stack))) ;?
    val))
...
(defun calc-command (tok)
  "Give a command token, perform the appropriate action."
  (cond ((equal tok "c")
         (calc-clear-stack))
        ((equal tok "=")
...
;; As side effects, advance point on past the token,
;; and set name of function to use on process the token."
;;   (let (tok)
;;     (cond ((looking-at calc-number-regexp)
;;            (goto-char (match-end 0))
;;            (setq calc-proc-fun 'calc-push-number))
;;           ((looking-at calc-operator-regexp)
;;            (message calc-operator-regexp)
;;            (forward-char 1)
...
and set name of function to use to process the token."
  (let (tok)
    (cond ((looking-at calc-number-regexp)(goto-char (match-end 0))
           (setq calc-proc-fun 'calc-push-number))
          ((looking-at calc-operator-regexp)
           (forward-char 1)
           (setq calc-proc-fun 'calc-operate))
...
Press all tokens on a input line."
  (interactive)
  ;;  (beginning-of-line)
  (goto-char (point-min))
  (while (not (eolp))
...
Understands the arithmetic operator +, -, *, / and %,
plus the following command:
     c    clear stack
     =    print top of stack
...
whitespace is necessary, except to seperate numbers."
  (interactive)
