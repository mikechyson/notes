...
templates by appending \"-extension\" to this filename, e.g. a Java specifictemplate would be file-template-java.")
...
over an extension-free, generic template."
  (let ((path (file-name-directory (buffer-file-name)))
        ...
        attempt result)(while (and (not result) (> (length path) 0))
      ...
      (if (and (find-template-file)
          ...
          (progn (buffer-disable-undo)
                 ...
                 (let ((the-list template-replacements-alist))
                   (while the-list
                     (goto-char (point-min))(replace-string (car (car the-list))
