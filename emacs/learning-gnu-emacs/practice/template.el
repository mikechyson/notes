;; Routines for generating smart skeletal templates ofr files.
(defvar template-file-name "file-template"
  "*The name of the file to look for when a find-file request fails.
If a file with the name specified by this variable exists, offer to
use it as a template for creating the new file. You can also have 
mode-specific templates by appending \"-extension\" to this filename,
e.g. a Java specific template would be file-template-java.")
...
(defvar template-replacement-alist
  ...
  "A list which specifies what substitutions to perform upon loading 
a template file. Each file element consists of a string, which is 
the target to be replaced if it is found in the template, paired with
a function, which is called to generate the replacement value for the
string.")
...
(defun template-file-not-found-hook ()
  "Called when a find-file command has not been able to find the
specified file in the current directory. See if it makes sense 
to offer to start it based on a template."
  (condition-case nil
      (if (and (find-template-file)
               (y-or-n-p "Start with tempalte file? "))
          (progn
            (buffer-disable-undo)
...
(or (memq 'template-file-not-found-hook find-file-not-found-hooks)
    (setq find-file-not-found-hooks (append find-file-not-found-hooks '(template-file-not-found-hook))))


(defun template-insert-include-once ()
  "Returns preprocessor directives such that the file will be included 
only once during a compilation process which includes it an abitrary
number of times."
...
(defun template-insert-java-package ()
  "Insert an appropriate Java package directive based on the path to
the current file name (assuming that it is in the com, org or net
subtree). If no recognizable package path is found, insert nothing."
  (let ((name (file-name-directory (buffer-file-name)))
        result)
    (if (string-match "/\\(com\\|org\\|net\\)/.*/$" name)
        (progn
          (setq result (substring name (+ (match-beginning 0) 1) (- (match-end 0) 1)))
...
(defun template-insert-class-name ()
  "Insert the name of the java class being defined in the current file,
